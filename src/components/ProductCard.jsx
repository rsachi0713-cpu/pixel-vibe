...
const ProductCard = ({ product }) => {
  const { id, title, type, price, image, isFree, is_free, badge } = product;
  const freeStatus = isFree || is_free;
  ...
  return (
    <div className={`p-4 rounded-xl flex flex-col h-full bg-dark-800/60 border border-gray-800 transition-all duration-300 hover:-translate-y-1 ${currentStyle}`}>
      <div className="relative aspect-square md:aspect-video lg:aspect-square overflow-hidden rounded-lg mb-4 bg-dark-900 group watermark-container">
        <img 
        ...
        {badge && (
          <div className="absolute top-2 left-2 bg-dark-900/80 backdrop-blur-sm border border-gray-700 text-xs font-bold px-2 py-1 rounded z-30">
             {badge}
          </div>
        )}
        <div className="absolute inset-0 bg-gradient-to-t from-dark-900 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center gap-3 z-40">
      ...
      <div className="flex-grow flex flex-col justify-between">
        ...
        <div className="flex items-center justify-between mt-auto">
          ...
          <Link to={`/product/${id}`} className={`px-4 py-1.5 rounded-full text-sm font-medium transition-colors ${
            freeStatus ? 'bg-accent/20 text-accent hover:bg-accent hover:text-white' : 'bg-primary/20 text-primary hover:bg-primary hover:text-white'
          }`}>
            View Details
          </Link>
        </div>
